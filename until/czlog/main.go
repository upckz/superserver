...
func (l Logger) doPrintf(level LogLevel, format string, v ...interface{}) {
	...
	if level >= l.level {
		//t := time.Now().UnixNano() / 1e6 % 1000
		funcName, _, lineNum := getRuntimeInfo()
		format = fmt.Sprintf("%5s [%s:%d] - %s",tagName[level], path.Base(funcName),  lineNum, format)
...
func (l Logger) doPrintln(level LogLevel, v ...interface{}) {
	...
	if level >= l.level {
		...
		funcName, _, lineNum := getRuntimeInfo()
		prefix := fmt.Sprintf("%5s [%s:%d] - ",tagName[level], path.Base(funcName),  lineNum)
